...
st.markdown("Predict all remaining results for the top 6 teams.")
...
for i, team in enumerate(tracked_teams.keys()):
    with tabs[i]:
        ...
        for fixture_index, match in enumerate(fixtures):
            ...
            owner = match[0]  # The first team in the fixture owns the input
...
df["Movement"] = [get_arrow(row["Team"], idx) for idx, row in df.iterrows()]

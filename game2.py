    ...
    with tabs[i]:
        ...
        for fixture_index, match in enumerate(fixtures):
            if team in match:
                ...
                # Stable ownership and unique key with index
                key_team1, key_team2 = sorted([team, opponent])
                match_key = f"{key_team1} vs {key_team2}"
                widget_key = f"{match_key}::{fixture_index}"  # 🔐 Unique key per fixture

                if team == key_team1:
                    # Owner tab shows radio
                    result = st.radio(
